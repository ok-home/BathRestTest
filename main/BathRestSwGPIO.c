...
#define MAXFIXINT 2                       // минимум еще несколько интервалов для особо плохих кнопок
...
static void IRAM_ATTR BathLightSw_isr_handler(void *arg)
{
    ...
    vTaskNotifyGiveFromISR(BathLightSwDebounce);
}
static void IRAM_ATTR RestLightSw_isr_handler(void *arg)
{
    ...
    vTaskNotifyGiveFromISR(RestLightSwDebounce);
}
...
void LightISRSetup()
{
    xTaskCreate(BathLightSwDebounce, "blsw", 2000, NULL, 1, NULL);
    xTaskCreate(RestLightSwDebounce, "rlsw", 2000, NULL, 1, NULL);
