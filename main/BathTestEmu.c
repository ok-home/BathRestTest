...
// проверка проводов датчика расстояния
...
//disable interrupt
    ...
    io_conf.pin_bit_mask = (1UL<<GPIO_HUM_SDA_INOUT);
    ...
    gpio_config(&io_conf);
    for(;;){
    gpio_set_level(GPIO_HUM_SDA_INOUT,1);
    vTaskDelay(1000/portTICK_RATE_MS);
    gpio_set_level(GPIO_HUM_SDA_INOUT,0);
    vTaskDelay(1000/portTICK_RATE_MS);
    
    }


/*  // закрыт блок датчиков движения
    //disable interrupt
    io_conf.intr_type = GPIO_PIN_INTR_DISABLE;
    //set as output mode
    io_conf.mode = GPIO_MODE_OUTPUT;
